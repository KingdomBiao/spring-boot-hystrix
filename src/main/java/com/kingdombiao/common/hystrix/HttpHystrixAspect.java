...
import com.kingdombiao.hystrix.config.SetterFactory;
...
public class HttpHystrixAspect {
	...
	@Around(value="execution (* com.kingdombiao.service.impl.*.*(..)) && @annotation(hystrixCommand)")
	...
	public class HttpHystrix extends com.netflix.hystrix.HystrixCommand<Object>{
        ...
        protected HttpHystrix(ProceedingJoinPoint pjp, String className, String methodName, HystrixCommand hystrixCommand) {
            // 同一个groupKey共用同一个线程池
            super(SetterFactory.createSetter("spring-boot-hystrix", className, methodName));
            this.pjp = pjp;
